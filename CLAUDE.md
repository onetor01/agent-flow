...
- **CompleteTask.content 作为 next agent 首条消息回显** → [reducer agentComplete 分支](src/common/flowRunState.ts):创建新 `AgentRun` 时 `messages` 预置一条 user `aiMessage`(content = `nextAgent.no_input ? '执行任务' : data.content`),与 `FlowRunner.doOnCompleteTask` 喂 SDK 的 `nextInitMessage` 同源,改链路两端同改
