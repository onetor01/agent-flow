...
- **resumeSessionId 启动路径**：`flow.command.flowStart` 携带 `resumeSessionId` 时 reducer 在 resume 模式下保留既有 sessions / answered\* / shareValues 不清空；flowStart signal 命中已有 sessionId 时复用而非追加。fork 出的新 Flow 在用户首次发消息时走 `startFlow + resumeSessionId` 路径接续运行；[ChatDrawer.onSend](src/webview/components/ChatDrawer.tsx) 检测 runId 缺失时会自动取该 agent 最近 session 的 sessionId 作为 resumeSessionId。
