...
`FlowPhase` / `AgentPhase` 同构:`idle | starting | running | result | interrupted | awaiting-question | awaiting-tool-permission | completed | stopped | error`,共用 `aggregatePhase(runs)`。Phase 不存字段,由 [getRunPhase / getAgentPhase / getFlowPhase](src/common/flowRunState.ts) 推断。

`FlowRunState` 数据结构、`MessageEffect` 5 个 reason、终态守卫见 [flowRunState.ts](src/common/flowRunState.ts) 顶部注释。silent_task 仅放行 `agent-error` / `flow-completed`(详见 `pushEffect`)。
...
- **silent_task**:无人值守。AskUserQuestion 自动应答 / result 自动续轮 / canUseTool 默认 deny / `pushEffect` 仅放行 error|completed / 暴露 `terminateTask` MCP / AgentEditor 首次切换弹 warning。详见 [ClaudeExecutor.ts](src/extension/FlowRunnerManager/FlowRunner/ClaudeExecutor.ts) 与 reducer `pushEffect`
...
- **fork 切片 uuid 用 SDK transcript 实际值** → [handleFork](src/extension/index.ts):末尾 `getSessionMessages` 拿新 session 真实 transcript,把切片末端 user/assistant uuid 替换为 SDK remap 后新值,否则二次 fork / turn_end fork 报 forkSession failed
