...
- **require_confirm 按 output 粒度配置，silent_task 不自动应答** → [ClaudeExecutor.canUseTool](src/extension/FlowRunnerManager/FlowRunner/ClaudeExecutor.ts):AgentComplete 入参 `output_name` 命中的 Output 若 `require_confirm===true` 才挂 pending、fire `onCompleteConfirmRequest`；无 outputs / 未匹配 output 直接放行；chat 模式不挂 AgentComplete 故不进入。silent_task 模式命中此路径同样挂起，不自动放行，必须用户人工确认。
- **plan_mode 映射 SDK permissionMode** → [ClaudeExecutor.ts](src/extension/FlowRunnerManager/FlowRunner/ClaudeExecutor.ts):`agent.plan_mode ? 'plan' : 'default'`；plan 下 Claude 注入的系统提示词倾向改变且无法写文件,AgentEditor「Plan模式」Switch 控制,字段语义见 [src/common/index.ts](src/common/index.ts) `plan_mode` 注释
- **base_url / api_key Agent 覆盖 Flow,空字符串视为不覆盖** → [FlowRunner](src/extension/FlowRunnerManager/FlowRunner/index.ts) `agent.base_url || flow.base_url`(空串 falsy 落到 flow);[ClaudeExecutor.createQuery](src/extension/FlowRunnerManager/FlowRunner/ClaudeExecutor.ts) 仅在任一非空时设置 `options.env`,**必须** spread `process.env` 否则 SDK 子进程丢 PATH/HOME;两端均不覆盖时 env 字段不下发,SDK 默认继承 process.env
