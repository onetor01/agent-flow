...
- **silent_task**：无人值守。
  - AskUserQuestion 自动以 `"自行处理"` 应答 + `behavior: 'allow'`，同步 `events.onAnswerQuestion` 走 `flow.signal.answerQuestion`，与人工回答展示路径一致。
  - 每轮 `result/success` 收到后（未 `pendingCompleteResult` 且未 disposed）构造 `buildSilentContinueMessage`（content `"自行处理"`），先 `events.onMessage` 手动 echo（SDK 不 mirror input stream user message），再 `userInputStream.push`。
...
- **ClaudeExecutor (runId, sessionId) 校验**：暴露 `runId` / `sessionId` getter + `matches(runId, sessionId)`。`FlowRunner` 不维护 `currentRunId/SessionId/AgentId`，`checkSession` 直接转发 `currentExecutor?.matches(...)`，避免切换时 webview 旧 sessionId 误派发到新 executor。
...
- **ChatPanel 跨 Flow 切换必须 unmount**：[ChatDrawer](src/webview/components/ChatDrawer/index.tsx) 给 ChatPanel 加 `key={flowId-agentId}`，避免 AskUserQuestionCard selections / motion.div ask-card key 在新旧 Flow 间被复用。
