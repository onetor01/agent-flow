...
领域定义与校验在 [src/common/index.ts](src/common/index.ts)。Flow 定义按作用域持久化到 `.agent-flows.json`：全局（`os.homedir()`）和项目（workspace root）；`FlowRunState` 仅内存，项目运行态写入全局文件 `workspaceRunStates[workspaceRoot]` 用于崩溃恢复,extension 端 [FlowRunStateManager](src/extension/FlowRunStateManager.ts) 镜像;UI 状态仅 webview。
...
- 项目文件只保存项目 flows；全局文件保存全局 flows 与 `workspaceRunStates`（按 workspaceRoot 分组的项目 `FlowRunState`） → [src/extension/index.ts](src/extension/index.ts)
...
- **killFlow vs flowStart 语义对照** → [flowRunState.ts](src/common/flowRunState.ts) `killFlow` / `flowStart` 分支:killFlow 保留 messages / shareValues;flowStart 追加新 run 并保留 messages / shareValues;shareValues 仅在 phase→completed 时清空
