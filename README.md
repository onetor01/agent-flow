...
- **无输入启动**：开启 `no_input` 的 Agent 在节点上显示启动按钮，点击后始终以"执行任务"为初始消息自动运行，无需手动输入。
...
- **Code 节点**：`node_type='code'` 的节点不走 AI SDK，而是把 `code` 字段当 `async function (input, values, runCommand) { ... }` 函数体执行。入参 `input` 为上游节点输出文本（`no_input` 时为 `'执行任务'`），`values` 为当前 Flow 全部 shareValues 快照（全量读不受授权约束），`runCommand` 为在 workspace 下执行 shell 命令的异步函数。返回值映射为 `{ output_name?, content?, values? }` 驱动下一跳，`values` 与现有 shareValues 合并。代码编辑器基于 CodeMirror，提供 `input` / `values.*` / `runCommand` 补全和 `Shift+Alt+F` 格式化。
